...
def parse_args():
    ...
    parser.add_argument('--data_path', type=str, default="./data/", help=('Path to the datasets. This should point to the location with `megadepth` and `imw2020-val` directories.'))
    parser.add_argument('--reward', choices=['epipolar', 'depth'], default='epipolar',  help='Reward criterion to use')
    parser.add_argument('--save-dir', type=str, default='artifacts',  help=('Path for saving artifacts (checkpoints and tensorboard logs). Will be created if doesn\'t exist'))
    ...
    parser.add_argument('--height', type=int, default=768, help='We train on images resized to (height, width)')
    parser.add_argument('--width', type=int, default=768,  help='We train on images resized to (height, width)')
    parser.add_argument('--train-scene-limit', type=int, default=1000, help=('Different scenes in the dataset have a different amount of covisible image triplets. We (randomly) subselect --train-scene-limit of them for training, to avoid introducing '
            'a data bias towards those scenes.'))
    parser.add_argument('--test-scene-limit', type=int, default=250, help=('Different scenes in the dataset have a different amount of covisible image triplets. We (randomly) subselect --test-scene-limit of them for validation to avoid '
            'to avoid introducing a bias towards those scenes.'))
    parser.add_argument('--n-epochs', type=int, default=50, help='Number of (pseudo) epochs to train for')
