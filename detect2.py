...
if __name__ == '__main__':
    ...
    parser.add_argument('--height', default=1088, type=int, help='rescaled height (px). If unspecified, image is not resized in height dimension')
    parser.add_argument('--width', default=1920, type=int, help='rescaled width (px). If unspecified, image is not resized in width dimension')
    parser.add_argument('--image-extension', default='jpeg', type=str, help='This script ill process all files which match `image-path/*.{--image-extension}`')
    ...
    parser.add_argument('--mode', choices=['nms', 'rng'], default='nms', help=('Whether to extract features using the non-maxima suppresion mode or through training-time grid sampling technique'))
    
    default_model_path = os.path.split(os.path.abspath(__file__))[0] + '/depth-save.pth'
    # parser.add_argument('--model_path', type=str, default="./checkpoint/epipolar-save.pth", help="Path to the model's .pth save file")
    parser.add_argument('--model_path', type=str, default="./artifacts2/save-29.pth", help="Path to the model's .pth save file")
    parser.add_argument('--detection-scores', action='store_true')
    
    parser.add_argument('--h5_path', type=str, default="./output2/h5_path_scratch_save29/", help=("Directory where keypoints.h5 and descriptors.h5 will be stored. This will be created if it doesn't already exist."))
    # parser.add_argument('--image_path', type=str, default="/workspace/mnt/storage/zhangjunkang/zjk1/data/sample_140_p1_pinhole/images/", help="Directory with images to be processed.")
    parser.add_argument('--image_path', type=str, default="/workspace/mnt/storage/zhangjunkang/zjk1/data/colmap_gaosu/sample_data/33.64.37.140_preset1/images/", help="Directory with images to be processed.")
