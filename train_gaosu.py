...
def parse_args():
    ...
    parser.add_argument('--data_path', type=str, default="/workspace/mnt/storage/zhangjunkang/zjk1/data/colmap_gaosu/sample_data/", help=('Path to the datasets. This should point to the location with `megadepth` and `imw2020-val` directories.'))
    parser.add_argument('--reward', choices=['epipolar', 'depth'], default='epipolar',  help='Reward criterion to use')
    parser.add_argument('--save-dir', type=str, default='artifacts2',  help=('Path for saving artifacts (checkpoints and tensorboard logs). Will be created if doesn\'t exist'))
    parser.add_argument('--batch-size', type=int, default=2, help='The size of the batch')
    parser.add_argument('--chunk-size', type=int, default=5000, help=('The number of batches in the (pseudo) epoch. We run validation and save a checkpoint once per epoch, as well as use them for scheduling the reward annealing'))
    parser.add_argument('--substep', type=int, default=1, help=('Number of batches to accumulate gradients over. Can be increased to compensate for smaller batches on GPUs with less VRAM'))
    parser.add_argument('--warmup', type=int, default=250, help=('The first (pseudo) epoch can be much shorter, this avoids wasting time.'))
    parser.add_argument('--height', type=int, default=544, help='We train on images resized to (height, width)')
    parser.add_argument('--width', type=int, default=960,  help='We train on images resized to (height, width)')
    parser.add_argument('--train-scene-limit', type=int, default=100, help=('Different scenes in the dataset have a different amount of covisible image triplets. We (randomly) subselect --train-scene-limit of them for training, to avoid introducing '
            'a data bias towards those scenes.'))
    parser.add_argument('--test-scene-limit', type=int, default=10, help=('Different scenes in the dataset have a different amount of covisible image triplets. We (randomly) subselect --test-scene-limit of them for validation to avoid '
            'to avoid introducing a bias towards those scenes.'))
    parser.add_argument('--n-epochs', type=int, default=30, help='Number of (pseudo) epochs to train for')
    parser.add_argument('--desc-dim', type=int, default=128, help='Dimensionality of descriptors to produce. 128 by default')
    # parser.add_argument('--load', type=str, default="./checkpoint/depth-save.pth", help='Path to a checkpoint to resume training from')
    parser.add_argument('--load', type=str, default=None, help='Path to a checkpoint to resume training from')
    # parser.add_argument('--load', type=str, default="./checkpoint/epipolar-save.pth", help='Path to a checkpoint to resume training from')
    parser.add_argument('--epoch-offset', type=int, default=0, help=('Start counting epochs from this value. Influences the annealing procedures, and is therefore useful when restarting from a checkpoint'))
