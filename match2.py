...
if __name__ == '__main__':
    ...
    parser.add_argument('--h5_path', type=str, default="./output2/h5_path_scratch_save29/", help=('Path to the .h5 artifacts directory (containing descriptors.h5 and keypoints.h5)'))
    parser.add_argument('--f16', action='store_true', help=('Compute distance matrices in half precision (offers a substantial speedup with Turing and later GPUs).'))
    parser.add_argument('--u16', action='store_true', help=('Store matches with as uin16. This won\'t work if you have more than ~65k features in an image, but otherwise saves disk space.'))
    parser.add_argument('--rt', type=float, default=None, help='Ratio test value. Leave unspecified to perform no ratio test')
    parser.add_argument('--save-threshold', type=float, default=-float('inf'), help=('Don\'t save matches between a pair of images if less than --save-threshold were found.'))
    parser.add_argument('--max-full-matrix', type=int, default=10000**2, help=('this is the biggest match matrix that will attempt to be computed allocated in memory. Matrices bigger than that will be split into chunks of at most this size. Reduce if your script runs out of memory.'))
...
if __name__ == '__main__':
    ...
    with h5py.File(os.path.join(args.h5_path, 'matches.h5'), 'w') as hdf:
        brute_match(described_samples, hdf)
